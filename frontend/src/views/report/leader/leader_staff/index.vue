    ...
    <div class="border-box d-flex flex-column ml-2">
      ...
      <div class="container pdf-content" v-show="true" ref="pdfContent">
        ...
        <table class="table table-bordered mt-4">
          <thead>
            <tr>
              <th v-for="(value, index) in ['Stt','Họ tên','Sdt','Email','Địa chỉ', 'Ngày sinh']" :key="index">
                {{ value }}
              </th>
            </tr>
          </thead>
  ...
  export default {
    ...
    setup() {
      ...
      store.countleaderStaff = await countElementReportLeaderStaff();
        ...
        const employee = await http_getAll(Employee)
        data.lengthEmpoyee =  employee.length;
        const tasks = await http_getAll(Task)
        data.items = tasks.filter((task, index) => {
          // console.log('task', task);
          if(task.leaderId == leaderId){
            return tasks.findIndex(value => value.Employee._id === task.Employee._id) === index;
          }
        })
      ...
      const toString = computed(() => {
        ...
        if (data.choseSearch == "name") {
          return data.items.map((value, index) => {
            return [value.nameEmployee].join("").toLocaleLowerCase();
          });
        } else if (data.choseSearch == "email") {
          return data.items.map((value, index) => {
            return [value.emailEmployee].join("").toLocaleLowerCase();
          });
        } else if (data.choseSearch == "phone") {
          return data.items.map((value, index) => {
            return [value.phoneEmployee].join("").toLocaleLowerCase();
          });
        } else {
          return data.items.map((value, index) => {
            return [
