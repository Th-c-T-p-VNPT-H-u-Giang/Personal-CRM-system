...
export default {
  ...
  setup(ctx) {
    ...
    const update = async (value_id) => {
      console.log("UPDATE", value_id);
      let data = await unitServices.getOne(value_id);
      const showSweetAlert = async () => {
        const { value: formValues } = await Swal.fire({
          title: "Thêm phòng mới",
          html: `
      <select id="my-select-center" class="swal2-input form-control ml-3 mx-2 mb-3 mx-2" style="width:92%">
    ...
    `,
        ...
        if (formValues) {
          ...
          if (documents.error) {
            alert_warning(`Bạn chưa chỉnh sửa  `, `${formValues.inputValue}`);
            return;
          }
    ...
    onBeforeMount(async () => {
      ...
      data.items = await unitServices.getAll();
    });
    return {
      data,
      setPages,
