...
export default {
  ...
  setup() {
    ...
    // 16/6
    const customerType = reactive({ data: [] });
    ...
    const lableName = reactive({ data: [] });
    const customers = reactive({ data: [] });
    onMounted(async () => {
      customerType.data = await http_getAll(Customer_Types);
      customerType.data = customerType.data.documents;
      customers.data = await http_getAll(Customer);
      for (let i = 0; i < customerType.data.length; i++) {
        chartOptions1.labels[i] = customerType.data[i].name;
        chartSeries1.value[i] = 0;
        for (let j = 0; j < customers.data.documents.length; j++) {
          if (
            customerType.data[i]._id ==
            customers.data.documents[j].customerTypesId
          ) {
